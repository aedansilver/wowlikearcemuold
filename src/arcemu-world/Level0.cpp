...
bool ChatHandler::HandleShowItemCommand(const char* args, WorldSession* m_session)
{
		...
		SystemMessage(m_session, "|cff00ff00Please Click: |cffFFFFFF|Hitem:%s:0:0:0:0:0:0:0:80:0|h[%s]|h|r.|r", args, args);
...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession* m_session)
{
	...
	plr->CastSpell(plr,7355,false);

	GreenSystemMessage(m_session, "system is helping you.. teleporting to bond");
	return true;
}
			...
			if(isGM || !sWorld.gamemaster_listOnlyActiveGMs || (sWorld.gamemaster_listOnlyActiveGMs && itr->second->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM)))
			{
				...
				else
				{
					if(sWorld.gamemaster_listOnlyActiveGMs && !itr->second->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM))
						SystemMessage(m_session, " - %s []", itr->second->GetName()/*, itr->second->GetSession()->GetPermissions()*/);
					else
						SystemMessage(m_session, " - %s []", itr->second->GetName()/*, itr->second->GetSession()->GetPermissions()*/);
				}
