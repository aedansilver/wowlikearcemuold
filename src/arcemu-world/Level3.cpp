...
bool ChatHandler::HandleAddVipLevelCommand(const char* args, WorldSession* m_session)
{
	char* pvalue;
	uint32 av;
	...
	if(!pvalue)
		...
		if(av < 0)
		{
		RedSystemMessage(m_session, "Values are invalid. must be numeric.");
		return true;
		}
		else if(av > 6)
		{
		RedSystemMessage(m_session, "Values are invalid. cannot be higher than vip 6.");
		return true;
		}

		CharacterDatabase.Query("UPDATE accounts SET `member_type`='%s' WHERE `acct`='%u'", args, target->GetSession()->GetAccountId());

		BlueSystemMessage(m_session, "Player [%s] with account [%s] set vip level \'%s\'.", target->GetName(), target->GetSession()->GetAccountName().c_str(), args);
		return true;
}

bool ChatHandler::HandleAddMGCommand(const char* args, WorldSession* m_session)
{
	char* pvalue;
	uint32 av;
	...
	if(!pvalue)
		...
		CharacterDatabase.Query("UPDATE accounts SET `magic_gold`=`magic_gold`+'%s' WHERE `acct`='%u'", args, target->GetSession()->GetAccountId());

		QueryResult * HisMGfind = CharacterDatabase.Query("SELECT magic_gold FROM accounts WHERE acct='%u'", target->GetSession()->GetAccountId());

		Field * HisMG = HisMGfind->Fetch();

		SystemMessage(m_session, "|cff00ff00Added %s mg to [%s]. Total: %s|r", args, target->GetName(), HisMG->GetString());
		return true;
}
...
bool ChatHandler::HandleBanCharacterCommand(const char* args, WorldSession* m_session)
{
	...
	if(pPlayer == NULL)
   {
      SystemMessage(m_session, "Offline players cannot be banned.");
		return true;
   }
   else if(pPlayer->GetSession()->CanUseCommand('z')) // is admin
   {
      SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's an admin.", pCharacter, pCharacter);
      return true;
   }
    else if(pPlayer->GetSession()->CanUseCommand('a')) // is high gm
   {
      SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a high GM.", pCharacter, pCharacter);
      return true;
   }

   			// check vip level

				QueryResult * result = WorldDatabase.Query("SELECT member_type FROM account.accounts WHERE acct='%u'", pPlayer->GetSession()->GetAccountId());

			if(!result)
				return true; // If is a void function. If it is a boolean, put return false;
				...
				do
			{
				...
				if (field[0].GetUInt32() == 1)
					{
						// VIP 1
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 1.", pCharacter, pCharacter);
						return true;
					}
				else if (field[0].GetUInt32() == 2)
					{
						// VIP 2
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 2.", pCharacter, pCharacter);
						return true;
					}
				else if (field[0].GetUInt32() == 3)
					{
						// VIP 3
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 3.", pCharacter, pCharacter);
						return true;
					}
				else if (field[0].GetUInt32() == 4)
					{
						// VIP 4
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 4.", pCharacter, pCharacter);
						return true;
					}
				else if (field[0].GetUInt32() == 5)
					{
						// VIP 5
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 5.", pCharacter, pCharacter);
						return true;
					}
				else if (field[0].GetUInt32() == 6)
					{
						// VIP 6
					SystemMessage(m_session, "You should not ban |Hplayer:%s|h[%s]|h since it's a vip 6.", pCharacter, pCharacter);
						return true;
					}
	else
	{
	...
	char msg[200];
	snprintf(msg, 200, "%sSYSTEM: [%s] has been banned by [%s] for [%s]. Reason: [%s]", MSG_COLOR_RED, pCharacter, m_session->GetPlayer()->GetName(), BanTime ? ConvertTimeStampToString(BanTime).c_str() : "ever", (pReason == NULL) ? "No reason." : pReason);
...
// custom banaccbychar
bool ChatHandler::HandleBanAccByChar(const char* args, WorldSession* m_session)
{
	...
	QueryResult* acctid = CharacterDatabase.Query("SELECT acct FROM characters WHERE name='%s'", pCharacter);
	if(!acctid)
	return false; // If is a void function. If it is a boolean, put return false;

	QueryResult* acctname = CharacterDatabase.Query("SELECT login FROM accounts WHERE acct='%s'", acctid->Fetch()->GetString());
	QueryResult* character = CharacterDatabase.Query("SELECT name FROM characters WHERE acct='%s' and `online`=1", acctid->Fetch()->GetString());

	if(character == NULL)
		return false;

	QueryResult * viplvl = WorldDatabase.Query("SELECT member_type FROM account.accounts WHERE acct='%s'",acctid->Fetch()->GetString());
	...
	uint32 banned = 1;
	char emptystring = 0;
	char banaccbycharmsg[1024];

	if (viplvl->Fetch()[0].GetUInt32() >= 1 && m_session->CanUseCommand('a') == false)
	{
		SystemMessage(m_session, "You should not ban %s since it's a vip %s.", character->Fetch()->GetString(), viplvl->Fetch()->GetString());
	return true;
	}
	else
	snprintf(banaccbycharmsg, 1024, "|cffD55858[%s]'s account has been banned by (%s).", character->Fetch()->GetString(), m_session->GetPlayer()->GetName());
	sWorld.SendWorldText(banaccbycharmsg);

	sLogonCommHandler.Account_SetBanned(acctname->Fetch()->GetString(), banned, &emptystring);
	sWorld.DisconnectUsersWithAccount(acctname->Fetch()->GetString(), m_session);
	return true;	
}
// end custom banaccbychar
...
bool ChatHandler::HandleModifyLevelCommand(const char* args, WorldSession* m_session)
{
	...
	{
		RedSystemMessage(m_session, "Cannot set level overcap.");
		return true;
	}
...
bool ChatHandler::HandleShutdownCommand(const char* args, WorldSession* m_session)
{
	...
	snprintf(msg, 500, "%s[%s]|r Shutdown by %s[%s]|r.", MSG_COLOR_LIGHTBLUE, sWorld.GetServerNamecK(), MSG_COLOR_LIGHTRED, m_session->GetPlayer()->GetName()/*, MSG_COLOR_LIGHTRED, (unsigned int)shutdowntime*/);
...
bool ChatHandler::HandleShutdownRestartCommand(const char* args, WorldSession* m_session)
{
	...
	snprintf(msg, 500, "%s[%s]|r Restart by %s[%s]|r. Will be back soon.", MSG_COLOR_LIGHTBLUE, sWorld.GetServerNamecK(), MSG_COLOR_LIGHTRED, m_session->GetPlayer()->GetName()/*, MSG_COLOR_LIGHTRED, (unsigned int)shutdowntime*/);
...
bool ChatHandler::HandleCancelShutdownCommand(const char* args, WorldSession* m_session)
{
	...
	snprintf(msg, 500, "Server %s%s|r cancelled by %s[%s]|r.", MSG_COLOR_LIGHTRED, (sMaster.m_restartEvent ? "restart" : "shutdown"), MSG_COLOR_LIGHTRED, m_session->GetPlayer()->GetName());
...
//	char* infos = new char[128];
	...
	BlueSystemMessage(m_session, "%s is connecting from account '%s'[%u] with permissions ''",
...
bool ChatHandler::HandleRehashCommand(const char* args, WorldSession* m_session)
{
	...
	*/
	...
	snprintf(msg, 250, "%s[%s]|r reloading config files.", MSG_COLOR_LIGHTBLUE, sWorld.GetServerNamecK());
