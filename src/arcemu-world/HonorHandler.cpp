...
void HonorHandler::AddHonorPointsToPlayer(Player* pPlayer, uint32 uAmount)
{
	...
	if(pPlayer->m_honorPoints > 50000000)
		pPlayer->m_honorPoints = 50000000;
...
bool ChatHandler::HandleAddArenaCommand(const char* args, WorldSession* m_session)
{
	uint32 ArenaAmount = args ? atol(args) : 1;
	Player* plr = getSelectedChar(m_session, true);
	if(plr == 0)
		return true;

	BlueSystemMessage(m_session, "Adding %u arena points to player %s.", ArenaAmount, plr->GetName());
	GreenSystemMessage(plr->GetSession(), "You have had %u arena points added to your character. (need relog)", ArenaAmount);

	HonorHandler::AddArenaPointsToPlayer(plr, ArenaAmount);
	return true;
}
