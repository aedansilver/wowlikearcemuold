...
void Player::GiveXP(uint32 xp, const uint64 & guid, bool allowbonus)
{
	...
	while(newxp >= nextlevelxp && newxp > 0)
	{
		...
		if(li == NULL) return;
		/*newxp -= nextlevelxp;*/
		newxp = li->XPToNextLevel;
		levelup = true;

		if(level > 9)
			/*AddTalentPointsToAllSpec( 1 );*/ /// custom
...
#define get_next_field fields[field_index++]
	...
	if(m_arenaPoints > 50000000) m_arenaPoints = 50000000;
	...
	if(m_honorPoints > 50000000) m_honorPoints = 50000000; // custom
...
void Player::OnPushToWorld()
{
	...
	*data << uint32(0);
	...
	if(m_FirstLogin)
	{
		...
		m_session->GetPlayer()->setLevel(startlevel);
		m_FirstLogin = false;
			// welcome new player customs stuff here
		GetSession()->GetPlayer()->SetTalentPointsForAllSpec(71);

		char welcomenewplayer[1024];

		if(class_ == DEATHKNIGHT)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Deathknight [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == WARRIOR)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Warrior [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == ROGUE)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Rogue [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == PRIEST)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Priest [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == MAGE)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Mage [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == WARLOCK)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Warlock [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == PALADIN)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Paladin [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == HUNTER)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Hunter [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == SHAMAN)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Shaman [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
		else if(class_ == DRUID)
		{
		snprintf(welcomenewplayer, 1024, " Welcome new %s Druid [|Hplayer:%s|h%s%s|h|r].", (m_session->GetPlayer()->getGender() ? "sister" : "brother"), m_session->GetPlayer()->GetName(), MSG_COLOR_WHITE, m_session->GetPlayer()->GetName());
		}
...
void Player::RepopRequestedPlayer()
{
	...
	if(!m_bg || (m_bg && m_bg->HasStarted()))
	{
		...
		if(pMapinfo != NULL)
		{
			...
			{
				/*RepopAtGraveyard(GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId());*/
				if(pMapinfo->mapid == 560) // xd then repop at the death coords
				{
					Player* plr = GetSession()->GetPlayer(); // custom blabla fixed this shit durn
					plr->EventTeleport(560, plr->GetPositionX(), plr->GetPositionY(), plr->GetPositionZ());
				}
			}
			else
			{
...
float Player::GetDodgeChance()
{
	...
	uint32 agi = GetStat( STAT_AGILITY ) /*/ 10.0f *//*/ level * 1.8*/; //custom dodge
	
	float tmp = 10.0f * ( baseCrit->val + agi * CritPerAgi->val ); // custom dodge works
...
void Player::UpdateChances()
{
	...
	tmp = min(max(tmp, 85.0f), 85.0f); // custom dodge 2
...
void Player::UpdateAttackSpeed()
{
	...
	else if(GetShapeShift() == FORM_BEAR || GetShapeShift() == FORM_DIREBEAR)
	{
		...
		/*SetHealth(GetMaxHealth());*/
		SetHealth(10000000);
		/*BroadcastMessage("fuck this bug %u", GetMaxHealth());*/ //custom
	}
...
void Player::Reset_Talents()
{
	...
	QueryResult * result = WorldDatabase.Query("SELECT member_type FROM account.accounts WHERE acct='%u'", Plr->GetSession()->GetAccountId());

	if(!result)
		return; // If is a void function. If it is a boolean, put return false;
 
		do
	{
	Field * field = result->Fetch();
	char vip_talent_reset[1024];

			 if (field[0].GetUInt32() == 1) // vip1
		{
			snprintf(vip_talent_reset, 1024, "VIP 1 [%s] gets 5 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(76);
		}
		else if (field[0].GetUInt32() == 2) // vip2
		{
			snprintf(vip_talent_reset, 1024, "VIP 2 [%s] gets 10 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(81);
		}
		else if (field[0].GetUInt32() == 3) // vip3
		{
			snprintf(vip_talent_reset, 1024, "VIP 3 [%s] gets 15 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(86);
		}
		else if (field[0].GetUInt32() == 4) // vip4
		{
			snprintf(vip_talent_reset, 1024, "VIP 4 [%s] gets 20 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(91);
		}
		else if (field[0].GetUInt32() == 5) // vip5
		{
			snprintf(vip_talent_reset, 1024, "VIP 5 [%s] gets 25 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(96);
		}
		else if (field[0].GetUInt32() == 6) // vip6
		{
			snprintf(vip_talent_reset, 1024, "VIP 6 [%s] gets 30 more talent points", m_session->GetPlayer()->getPlayerInfo()->name);
			m_session->GetPlayer()->SetTalentPointsForAllSpec(101);
		}
		sWorld.SendWorldWideScreenText(vip_talent_reset);
}while(result->NextRow());
...
void Player::CalcResistance(uint32 type)
{
	...
	pos += FlatResistanceModifierPos[type] / 200; // custom
	neg += FlatResistanceModifierNeg[type] / 200; // custom
	res = BaseResistance[type] + pos - neg;
	if(type == 0)res += m_uint32Values[UNIT_FIELD_STAT1] / 200.0f; //fix armor from agi (default * 2)
	if(res < 0)res = 0;
...
void Player::_AddSkillLine(uint32 SkillLine, uint32 Curr_sk, uint32 Max_sk)
{
	...
	Curr_sk = (Curr_sk > 500 ? 500 : (Curr_sk < 1 ? 1 : Curr_sk));
	Max_sk = (Max_sk > 500 ? 500 : Max_sk);
#else
...
//		/////////////////////////////////////////////////////////// Experience /////////////////////////////////////////////////////////////////////////////////////////////
//		Unit* uPlayer = pVictim->GetMapMgr()->GetUnit(pVictim->GetTaggerGUID());
//		Player* pPlayer = TO_PLAYER(uPlayer);
//
//	QueryResult * resultxprate = WorldDatabase.Query("SELECT member_type FROM account.accounts WHERE acct='%u'", pPlayer->GetSession()->GetAccountId());
//
//		if(pVictim->GetCreatedByGUID() == 0 && !pVictim->IsPet() && pVictim->IsTagged())
//		{
//			Unit* uTagger = pVictim->GetMapMgr()->GetUnit(pVictim->GetTaggerGUID());
...
//					{
//	
//					Field * field = resultxprate->Fetch();
//				if (field[0].GetUInt32() == 1) // vip 1 xp bonus
//					
//						uint32 xpt = xp *1.5;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//						/*if (xp == 0 || pTagger->getLevel() == 80)*/	
//				if (field[0].GetUInt32() == 2) // vip 2 xp bonus
//					
//						uint32 xpt = xp *2;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//				if (field[0].GetUInt32() == 3) // vip 3 xp bonus
//					
//						uint32 xpt = xp *2.5;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//				if (field[0].GetUInt32() == 4) // vip 4 xp bonus
//					
//						uint32 xpt = xp *3;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//				if (field[0].GetUInt32() == 5) // vip 5 xp bonus
//					
//						uint32 xpt = xp *3.5;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//					
//				if (field[0].GetUInt32() == 6) // vip 6 xp bonus
//					
//						uint32 xpt = xp *4;
//						uint32 xpbonus = xpt - xp;
//						uint32 xpb = xp;
//
//						pTagger->GiveXP(xpt, pVictim->GetGUID(), true);
//			}
//				/*else
//				{
//						pTagger->GiveXP(xpt, pVictim->GetGUID(), true);
