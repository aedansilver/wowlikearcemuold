...
void World::SendGamemasterMessage(WorldPacket* packet, WorldSession* self)
{
	...
	for(itr = m_sessions.begin(); itr != m_sessions.end(); itr++)
	{
		if(itr->second->GetPlayer() &&
		        ...
		        && itr->second != self)  // don't send to self!
		{
			//if(itr->second->CanUseCommand('g')) // custom
...
void World::Rehash(bool load)
{
	...
	// custom vip
	MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot6", 1);
	SetServerNamecK(Config.OptionalConfig.GetStringDefault("ServerCK", "ServerNamecK", "ServerNamecK").c_str());
	/*Player* player;*/
	//QueryResult * vipmember = CharacterDatabase.Query("SELECT member_type FROM accounts WHERE acct='1'"/*, player->GetSession()->GetAccountId()*/);
	//if(!vipmember)
	//	return; // If is a void function. If it is a boolean, put return false;
 //
	//		do
	//{
	//Field * field = vipmember->Fetch(); // if memberloot defined all
	//if (field[0].GetUInt32() == 1)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot1", 1);
	//else if (field[0].GetUInt32() == 2)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot2", 1);
	//else if (field[0].GetUInt32() == 3)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot3", 1);
	//else if (field[0].GetUInt32() == 4)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot4", 1);
	//else if (field[0].GetUInt32() == 5)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot5", 1);
	//else if (field[0].GetUInt32() == 6)
	//MemberLoot = Config.OptionalConfig.GetIntDefault("VipLootSettings", "MemberLoot6", 1);
	//else
	//MemberLoot = 1;
	//}while(vipmember->NextRow());
