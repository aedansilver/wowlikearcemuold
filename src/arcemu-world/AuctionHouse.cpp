...
void Auction::SaveToDB(uint32 AuctionHouseId)
{
	...
	QueryResult * quality = WorldDatabase.Query("SELECT quality FROM items WHERE entry='%u'", pItem->GetEntry());

	QueryResult * resultitem = WorldDatabase.Query("SELECT name1 FROM items WHERE entry='%u'", pItem->GetEntry());

	QueryResult * resultchar = CharacterDatabase.Query("SELECT name FROM characters WHERE guid='%u'", Owner);

	char worldAh[1024];

	if(!resultchar && !resultitem)
	return; // If is a void function. If it is a boolean, put return false;

	do
		{
			Field * fieldquality = quality->Fetch();
			Field * fielditem = resultitem->Fetch();
			Field * fieldchar = resultchar->Fetch();
		{
			if (fieldquality[0].GetUInt32() == 0)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cff9D9D9D[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 1)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cffFFFFFF[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 2)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cff1eff00[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 3)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cff0070dd[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 4)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cffa335ee[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 5)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |cffff8000[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 6)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |c00fce080[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else if (fieldquality[0].GetUInt32() == 7)
				{
					snprintf(worldAh, 1024, "%s [AUCTION] [|Hplayer:%s|h%s|h] SELL |c00fce080[|Hitem:%u|h%s|h]|r %sprice %u gold|r", MSG_COLOR_AHYELLOW, fieldchar->GetString(), fieldchar->GetString(), pItem->GetEntry(), fielditem->GetString(), MSG_COLOR_AHYELLOW, BuyoutPrice/10000);
				}
				else
		...
		sWorld.SendWorldText(worldAh);
	}while(resultchar->NextRow());

		// end of the custom stuff
}
