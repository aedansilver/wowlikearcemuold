...
bool HandleAnnounceCommand(BaseConsole* pConsole, int argc, const char* argv[])
{
	...
	snprintf(pAnnounce, 1024, "%sServer: |r%s", MSG_COLOR_LIGHTRED, outstr.c_str());
...
bool HandleWhisperCommand(BaseConsole* pConsole, int argc, const char* argv[])
{
	...
	snprintf(pAnnounce, 1024, "%sServer whisper: |r%s", MSG_COLOR_MAGENTA, outstr.c_str());
...
bool HandleKickCommand(BaseConsole* pConsole, int argc, const char* argv[])
{
	...
	ConcatArgs(outstr, argc, 1, argv);
	snprintf(pAnnounce, 1024, "%sServer:|r %s was removed from the server. Reason: %s.", MSG_COLOR_LIGHTBLUE, pPlayer->GetName(), outstr.c_str());
