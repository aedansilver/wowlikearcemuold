...
bool ChatHandler::HandleAnnounceCommand(const char* args, WorldSession* m_session)
{
	...
	if(sWorld.GMAdminTag)
	{
		if(m_session->CanUseCommand('z')) input2 += " "MSG_GM_ICON" <ADMIN> ";
			else if(m_session->CanUseCommand('a')) input2 += " "MSG_GM_ICON" HIGH <G.M> ";
		else if(m_session->CanUseCommand('g')) input2 += " "MSG_GM_ICON" <G.M> ";
		else input2 += " "MSG_GM_ICON" <G.M> ";
	}
	...
	sWorld.SendWorldRaidWarningText(msg); // send message 1
	sWorld.SendWorldText(msg); // send message 2
...
bool ChatHandler::HandleWAnnounceCommand(const char* args, WorldSession* m_session)
{
	...
	if(sWorld.GMAdminTag)
	{
		if(m_session->CanUseCommand('z')) input3 += " "MSG_GM_ICON" <ADMIN> ";
			else if(m_session->CanUseCommand('a')) input3 += " "MSG_GM_ICON" HIGH <G.M> ";
		else if(m_session->CanUseCommand('g')) input3 += " "MSG_GM_ICON" <G.M> ";
		else input3 += " "MSG_GM_ICON" <MOD> ";
	}
	...
	sWorld.SendWorldRaidWarningText(pAnnounce); // send message 1
	sWorld.SendWorldText(pAnnounce); // send message 2
...
bool ChatHandler::HandleKickCommand(const char* args, WorldSession* m_session)
{
	...
	if(chr)
	{
		...
		char msg[200];
		snprintf(msg, 200, "%sSYSTEM: [%s] was kicked from the server by [%s]. Reason: [%s]", MSG_COLOR_RED, chr->GetName(), m_session->GetPlayer()->GetName(), kickreason.c_str());
