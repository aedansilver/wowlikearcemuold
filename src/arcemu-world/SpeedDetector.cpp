...
void SpeedCheatDetector::ReportCheater(Player* _player)
{
	...
	_player->BroadcastMessage("Speedhack detected. In case server was wrong then make a report how to reproduce this case. You will be logged out in 1 seconds.");
	...
	sEventMgr.AddEvent(_player, &Player::_Kick, EVENT_PLAYER_KICK, 1000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	char speedhackchat[1024];
	snprintf(speedhackchat, 1024, "|cffD55858[WARNING] [|Hplayer:%s|h%s|h] speedhack detected!|r", _player->GetName(), _player->GetName());
	sWorld.SendWorldText(speedhackchat);
