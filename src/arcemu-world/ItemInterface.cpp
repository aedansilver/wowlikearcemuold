...
int8 ItemInterface::CanEquipItemInSlot(int8 DstInvSlot, int8 slot, ItemPrototype* proto, bool ignore_combat /* = false */, bool skip_2h_check /* = false */)
{
	...
	if((slot < INVENTORY_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET) || (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET))
	{
		...
		//custom vip items
		QueryResult * resultV = CharacterDatabase.Query("SELECT member_type FROM accounts WHERE acct='%u'", m_pOwner->GetSession()->GetAccountId());
		Field * member = resultV->Fetch();

		if(proto->PageId == 99999)
			if (member[0].GetUInt32() < 1) // need vip 1
			return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if(proto->PageId == 99991)
			if (member[0].GetUInt32() < 2) // need vip 2
			return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if(proto->PageId == 99992)
			if (member[0].GetUInt32() < 3) // need vip 3
			return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if(proto->PageId == 99993)
			if (member[0].GetUInt32() < 4) // need vip 4
			return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if(proto->PageId == 99994)
			if (member[0].GetUInt32() < 5) // need vip 5
			return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
		
		if(proto->PageId == 99995)
			if (member[0].GetUInt32() < 6) // need vip 6
		return INV_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
