...
class LuaUnit
{
	public:
		...
		static int SpawnCreature(lua_State* L, Unit* ptr)
		{
			...
			pCreature->AddToWorld(ptr->GetMapMgr()); // FIXED: Assert triggered when the SpawnCreature Lua command is called …
			if(duration)
		...
		static int SetNPCFlags(lua_State* L, Unit* ptr)
		{
			...
			ptr->SetUInt32Value(UNIT_NPC_FLAGS, flags); // FIXED: SetNPCFlags Lua command will now work properly.
			return 0;
		}
